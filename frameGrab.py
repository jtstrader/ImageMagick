...
def edge(count):
    ...
    subprocess.call(f'convert {count}.png -edge 5 ./Converted/{count}_ed.png', shell=True)
...
def main():
    ...
    count = 0
    if int(photoCount) >= 100:
        global keepOrig
        keepOrig = False # delete original photos if grabbing 100 or more
        delCount = 0
    for i in range(int(photoCount)):
        ...
        applyEffect(count, 0)
        if not keepOrig:
            subprocess.call(f'del {count}.png', shell=True)

        # delete extremely old photos to prevent overflow of photos
        if count >= 100:
            subprocess.call(f'del {delCount}.png', shell=True)
            delCount += 1
